...
token_response=$(curl 'http://169.254.169.254/metadata/identity/oauth2/token?api-version=2018-02-01&client_id=$identity_client_id&resource=https%3A%2F%2Fvault.azure.net' -H Metadata:true )
echo "token response: $token_response"
# should examine response for .error and abort

assigned_identity_token=$(jq -r '.access_token' <<< "$token_response")
echo "retrieved identity token: $assigned_identity_token"
results=$(curl $secret_id?api-version=7.2 -H "Authorization: Bearer $assigned_identity_token" -H "Content-Type: application/json")
echo "retrieved secret: $results"
